...
- **[当前学习：1B 可靠文档加载器](docs/lessons/01b-structured-loading.md)** — 1A 已完成，正在学习格式校验与短路求值。
- [环境准备与依赖分层](docs/course/setup.md) — 不要一次安装全部框架。
- [实际验证结果与未验证事项](docs/course/verification.md) — 不把未运行内容称为成功。
- [整体项目路线与技术取舍](docs/roadmap.md)。

**当前进度：1A 已完成，1B 进行中（已完成短路、非空校验对照及 Document 职责区分）。** 提前备齐教材，不代表后续阶段已自动通过；完成一阶段后，导师仍会询问是否继续。

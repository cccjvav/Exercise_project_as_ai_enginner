...
- **[当前节点：阶段 1 验收记录](docs/reviews/phase-01-evidence-baseline.md)** — 1A–1E 基础目标已完成，等待确认是否进入阶段 2。
- [环境准备与依赖分层](docs/course/setup.md) — 不要一次安装全部框架。
- [实际验证结果与未验证事项](docs/course/verification.md) — 不把未运行内容称为成功。
- [整体项目路线与技术取舍](docs/roadmap.md)。

**当前进度：阶段 1 基础验收通过；阶段 2 尚待学习者确认。** 提前备齐教材，不代表后续阶段已自动通过；完成一阶段后，导师仍会询问是否继续。

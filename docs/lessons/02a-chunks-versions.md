...
- **学习进度：** 进行中；学习者于 2026-09-17 确认进入阶段 2。不重叠分块、重叠窗口与步长预测均已答对；重叠重复量实验已反馈通过，取舍解释方向正确并获术语补充；来源、版本与更新尚未验收。
- **前置理解：** 阶段 1；理解文件身份与评测
...
概念检查（待回答）：同一份 doc 拆出的四个块，它们的 `parent_id` 应该相同还是不同？为什么？本轮未回答，不提前归档为已掌握。

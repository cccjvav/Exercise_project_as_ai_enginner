...
- **学习进度：** 进行中；已完成的题目与实操见文末归档。学习者反馈正常目录加载符合预期；真实加载器的错误路径待验证，整课尚未验收。
...
**预期观察（不是已完成记录）：** 抛出 `ValueError: broken.md: 第一行必须是 # 标题`。`## ` 不匹配我们要求的精确前缀 `# `。注意：Markdown 的二级标题语法本身合法，只是不满足本加载器规定的“文件第一行必须是一级标题”。
...
运行后反馈两次结果是否符合预期。目标是验证“错误输入被拒绝，修正输入后通过”，不是把所有报错都当成程序坏了。`artifacts/` 已被 Git 忽略，不会把这些临时练习资料当成正式语料提交。

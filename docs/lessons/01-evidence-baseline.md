...
**阶段 1（1A–1E）基础验收通过，学习者已确认进入阶段 2，当前学习 [2A](02a-chunks-versions.md)。** [查看验收依据与复现记录](../reviews/phase-01-evidence-baseline.md)。 本页是阶段全景和后续行为契约，不要求一次学完；检索、指标、测试的完整示例和逐行讲解已提前写好，见 [全部课程](../course/index.md)，现在不必一次学完。
...
|---|---|---|---|
| 1A（已完成）读取一份文档 | 已提供 `examples/01_read_document.py`；路径、UTF-8、拆行、字段提取 | 按步骤只替换文件名，读取第二份手册 | 先预测 ID/标题/来源，再运行核对 |
| [1B](01b-structured-loading.md)（已完成）可靠加载 | 完整的 Document 数据类、单文件函数、目录遍历与输入校验 | 在临时文件上删除标题标记，观察并解释报错 | 能区分“文件读不到”和“文件内容不合约定” |
| [1C](01c-lexical-search.md)（已完成）检索与排序 | 词表提取、集合交集、分数、稳定排序 | 改问题措辞与 k；预测命中，观察同义表达漏检 | 解释高分不等于有答案，以及扩大 k 的限制 |
| [1D](01d-cli-tests.md)（基础目标已完成）CLI 与测试 | 完整打包配置、argparse、JSON 输出、pytest 示例 | 按给定步骤加一条边界测试，先看失败再修复 | 能说明测试阻止了哪一种错误，而不只说“绿了” |
| [1E](01e-evaluation.md)（基础目标已完成）评测与反思 | 完整 JSONL 评测脚本，逐行讲解 Recall@k 与无答案题处理 | 切换 k=1/3；增加一条“有关键词但无答案”的反例 | 解释数据结果与指标局限，不用答案字段作弊 |

1A–1E 的参考实现与课程都已备齐，课程索引提供链接。学习者已确认继续阶段 2，从 2A 开始；不必为了教材齐备而提前安装所有依赖。

每次关键实操都采用相同结构：**打开哪个文件 → 修改哪一处 → 为什么改 → 运行什么命令 → 预期看到什么 → 若不同如何定位**。先小变式再工程化，不把查打包配置当成第一道门槛。

指定算法在当前 8 条开发题上的设计预期仍是：6 条可回答题命中 5 条，Recall@1 与 Recall@3 均为 5/6；2 条无答案题空返回。该基线已有导师参考复核和学习者运行反馈；具体验收依据见阶段记录，不以导师运行成功替代学习者理解。这不是生成式问答准确率。q06 的金标准依赖讨论 Webhook 的上下文；它是受限的同义表达演示，不能被用来证明系统会处理任意模糊提问。
...
阶段末已按模板汇总至[阶段 1 验收记录](../reviews/phase-01-evidence-baseline.md)，包括：
...
**review 与继续确认均已完成。** 当前进入 2A；q06/q09 保留在[跨阶段回归跟踪](../course/regression-cases.md)，不因为验收通过就视作已经解决。

...
**当前学习 [1B：可靠文档加载器](01b-structured-loading.md)。1A 已完成；1B 已完成短路/标题题、正文非空对照实操及 Document 职责区分题，其余内容尚未验收。** 本页是阶段全景和后续行为契约，不要求一次学完；检索、指标、测试的完整示例和逐行讲解已提前写好，见 [全部课程](../course/index.md)，现在不必一次学完。
...
| [1B](01b-structured-loading.md)（进行中）可靠加载 | 完整的 Document 数据类、单文件函数、目录遍历与输入校验 | 在临时文件上删除标题标记，观察并解释报错 | 能区分“文件读不到”和“文件内容不合约定” |
| [1C](01c-lexical-search.md)（待学）检索与排序 | 词表提取、集合交集、分数、稳定排序 | 改问题措辞与 k；预测命中，观察同义表达漏检 | 解释高分不等于有答案，以及扩大 k 的限制 |
| [1D](01d-cli-tests.md)（待学）CLI 与测试 | 完整打包配置、argparse、JSON 输出、pytest 示例 | 按给定步骤加一条边界测试，先看失败再修复 | 能说明测试阻止了哪一种错误，而不只说“绿了” |
| [1E](01e-evaluation.md)（待学）评测与反思 | 完整 JSONL 评测脚本，逐行讲解 Recall@k 与无答案题处理 | 切换 k=1/3；增加一条“有关键词但无答案”的反例 | 解释数据结果与指标局限，不用答案字段作弊 |

1A–1E 的参考实现与课程都已备齐，课程索引提供链接。当前从 1A 进入 1B，不必为了教材齐备而提前安装所有依赖。

每次关键实操都采用相同结构：**打开哪个文件 → 修改哪一处 → 为什么改 → 运行什么命令 → 预期看到什么 → 若不同如何定位**。先小变式再工程化，不把查打包配置当成第一道门槛。

指定算法在当前 8 条开发题上的设计预期仍是：6 条可回答题命中 5 条，Recall@1 与 Recall@3 均为 5/6；2 条无答案题空返回。本轮已用参考实现复核该基线；这不是学习者已通过验收，也不是生成式问答准确率。q06 的金标准依赖讨论 Webhook 的上下文；它是受限的同义表达演示，不能被用来证明系统会处理任意模糊提问。

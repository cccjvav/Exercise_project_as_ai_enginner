...
**当前学习 [1C：词法检索](01c-lexical-search.md)。1A、1B 已完成，阶段 1 尚未整体验收。** 本页是阶段全景和后续行为契约，不要求一次学完；检索、指标、测试的完整示例和逐行讲解已提前写好，见 [全部课程](../course/index.md)，现在不必一次学完。
...
| [1C](01c-lexical-search.md)（进行中）检索与排序 | 词表提取、集合交集、分数、稳定排序 | 改问题措辞与 k；预测命中，观察同义表达漏检 | 解释高分不等于有答案，以及扩大 k 的限制 |
...
1A–1E 的参考实现与课程都已备齐，课程索引提供链接。当前进入 1C，不必为了教材齐备而提前安装所有依赖。

...
**当前学习 [1B：可靠文档加载器](01b-structured-loading.md)。1A 已完成；1B 已答对空列表短路题与有效标题变式，其余内容尚未验收。** 本页是阶段全景和后续行为契约，不要求一次学完；检索、指标、测试的完整示例和逐行讲解已提前写好，见 [全部课程](../course/index.md)，现在不必一次学完。

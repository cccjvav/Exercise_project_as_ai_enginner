...
**当前学习 [1E：检索评测](01e-evaluation.md)。1A–1D 基础目标已完成，阶段 1 尚未整体验收。** 本页是阶段全景和后续行为契约，不要求一次学完；检索、指标、测试的完整示例和逐行讲解已提前写好，见 [全部课程](../course/index.md)，现在不必一次学完。
...
| [1E](01e-evaluation.md)（进行中）评测与反思 | 完整 JSONL 评测脚本，逐行讲解 Recall@k 与无答案题处理 | 切换 k=1/3；增加一条“有关键词但无答案”的反例 | 解释数据结果与指标局限，不用答案字段作弊 |

1A–1E 的参考实现与课程都已备齐，课程索引提供链接。当前进入 1E，不必为了教材齐备而提前安装所有依赖。

...
**当前学习 [1D：工程配置与测试](01d-cli-tests.md)。1A–1C 已完成，阶段 1 尚未整体验收。** 本页是阶段全景和后续行为契约，不要求一次学完；检索、指标、测试的完整示例和逐行讲解已提前写好，见 [全部课程](../course/index.md)，现在不必一次学完。
...
| [1D](01d-cli-tests.md)（进行中）CLI 与测试 | 完整打包配置、argparse、JSON 输出、pytest 示例 | 按给定步骤加一条边界测试，先看失败再修复 | 能说明测试阻止了哪一种错误，而不只说“绿了” |
| [1E](01e-evaluation.md)（待学）评测与反思 | 完整 JSONL 评测脚本，逐行讲解 Recall@k 与无答案题处理 | 切换 k=1/3；增加一条“有关键词但无答案”的反例 | 解释数据结果与指标局限，不用答案字段作弊 |

1A–1E 的参考实现与课程都已备齐，课程索引提供链接。当前进入 1D，不必为了教材齐备而提前安装所有依赖。

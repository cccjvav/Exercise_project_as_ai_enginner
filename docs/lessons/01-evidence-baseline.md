...
**现在只学 [第一小课：读取一份文档](01a-read-document.md)。** 本页是阶段全景和后续行为契约，不要求一次学完；检索、指标、测试的完整示例和逐行讲解已提前写好，见 [全部课程](../course/index.md)，现在不必一次学完。
...
| [1B](01b-structured-loading.md)（待学）可靠加载 | 完整的 Document 数据类、单文件函数、目录遍历与输入校验 | 在临时文件上删除标题标记，观察并解释报错 | 能区分“文件读不到”和“文件内容不合约定” |
...
1A–1E 的参考实现与课程都已备齐，课程索引提供链接。当前仍只学 1A，不必为了教材齐备而提前安装所有依赖。

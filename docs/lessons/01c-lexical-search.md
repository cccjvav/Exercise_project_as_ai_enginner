...
- **学习进度：** 进行中；领域词提取与集合去重题、查询词覆盖率计算题已答对。评分的证据边界、排序和检索实操尚未验收。
...
- 按固定词表，两次都提取 `webhook` 和 `重试`，预期返回 `webhook-delivery`，分数均为 `1.0`。这是可复核的教学预期，不代表学习者已完成实操。
- 逐项检查原文能否分别支持“重试次数”和“重试收费”的回答。程序当前只返回候选原文，不会自动判定它是否足以回答。

完成后反馈两次的分数，以及“能否仅凭分数为 1 判断两个问题都有答案”的判断和理由。只需摘要，不必贴完整 JSON。不要补写不存在的价格信息来让两个问题都可回答。

...
### 1C-Q5：top-k 会补足数量或解决零命中吗？

**实操：** 依次运行以下三条查询，记录返回数量，再判断第三个问题增大 `k` 是否有用：

```bash
python -m evidencedesk.search --query "Webhook 密钥" --k 1
python -m evidencedesk.search --query "Webhook 密钥" --k 3
python -m evidencedesk.search --query "消息没送到还会再发吗？" --k 3
```

**学习者反馈：** “前两次符合预期，最后一次范围0个文档，此时就算把k增大，也不会影响结果，因为k只是限制了返回的文档的数量。”

**判断：** 正确。前两次与第三次的运行结果依据学习者反馈记录；增大 `k` 的部分是正确的推理，不冒充学习者已额外执行 `k=10`。

**标准结果：**

| 查询 | k | 返回数量 | 结果 |
|---|---|---|---|
| Webhook 密钥 | 1 | 1 | `api-key-policy`，分数 0.5 |
| Webhook 密钥 | 3 | 2 | `api-key-policy`、`webhook-delivery`，分数均为 0.5 |
| 消息没送到还会再发吗？ | 3 | 0 | `[]` |

**标准解析：**

1. 前两次查询词为 `{"webhook", "密钥"}`。两份相关词文档各覆盖其中一个词，分数均为 0.5；按 ID 升序打破平局，`api-key-policy` 在前。
2. 工单文档的词覆盖率为 0，被过滤，不为凑满 `k=3` 而补进结果。`k` 是返回数量上限，不是必须达到的数量。
3. 第三个问题不含固定词表中的任何词，因此 `query_terms` 是空集合，函数在评分前就返回空列表：
...
4. 调整为另一个合法正整数 `k` 不会增加查询词，也不会让程序理解“再发”和“重试”的语义关系，所以仍然零命中。这个提前返回也避免后续除以 `len(query_terms)` 时发生除零错误。

**复习要点：**

- 分清“没召回候选”与“已有候选被 top-k 截断”；只有后者可能靠增大 k 展示更多结果。
- 零命中不证明知识库一定没有相关信息。若上下文明确讨论 Webhook，“再发”可能与重试规则有关；单独出现“消息”时仍有歧义，不能未经澄清就假定是哪一种消息。
- 当前基线不了解同义表达；不能为了这道题硬编码答案或修改金标准。

### 1C 小课验收小结

**结论：本课基础目标通过，进入同一阶段内的 1D。**

依据：领域词去重、查询词覆盖率、分数与证据充分性的区别、分数加唯一 ID 的确定性排序均已正确回答或反馈；top-k 数量上限、零命中原因及增大 k 的局限也已解释清楚。

此结论依据聊天中的回答和运行反馈，不等于导师独立核验了学习者本机全部日志。尚未覆盖的参数类型、非法 k、CLI 错误输出等边界将在 1D 用测试继续展开；阶段 1 仍需完成 1D/1E 后整体验收。

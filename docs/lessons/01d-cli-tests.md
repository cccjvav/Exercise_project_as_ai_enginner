...
- **学习进度：** 进行中；最小 pytest 用例与大小写归一化回归实操已反馈通过，且能解释防住的错误；异常测试机制题已答对，并补充了“预期异常”与“测试失败”的术语区分；非法 k 参数化实操及合法值反例预测已通过；CLI 解析与业务范围校验的分工题已纠正并通过（保留原始误解）；正常空结果与参数错误的终端观察和区分题已通过；成功时检查 stderr 的目的已正确解释；CLI 自动捕获测试的运行结果尚未反馈，其余边界尚未验收。
...
## 当前跟做：自动捕获 CLI 的结果与错误（理解题已答，运行结果待反馈）
...
预期摘要包含 `2 passed`。学习者已正确解释成功时仍需检查 stderr 的目的，见 1D-Q6；尚未明确反馈本测试文件的运行摘要，因此运行验收仍待确认，不据概念回答推定已经执行。本例预期正常运行无警告或错误信息；并非所有 CLI 都禁止在成功时向 stderr 写诊断信息。
...
**待确认事项：** 请学习者反馈 `python -m pytest tests/test_learning_cli.py -q` 的实际摘要；若尚未运行，按上方步骤执行即可。此前两条手工 CLI 命令符合预期，不等于这个新测试文件也已执行。

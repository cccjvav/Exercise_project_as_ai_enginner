...
学习者已反馈两条命令的可见结果符合预期，并正确解释将参数错误伪装成正常空结果的后果，见 1D-Q5。仅凭终端显示不能独立确认两个输出通道和退出码；后续 subprocess 捕获与断言已获通过反馈，见 1D-Q7。

## 自动捕获 CLI 的结果与错误（已完成，保留步骤供复习）
...
预期摘要包含 `2 passed`。学习者已正确解释成功时仍需检查 stderr 的目的（1D-Q6），并随后明确确认本测试文件运行结果符合 `2 passed`（1D-Q7）。本例预期正常运行无警告或错误信息；并非所有 CLI 都禁止在成功时向 stderr 写诊断信息。
...
### 1D-Q6：成功时为什么还要检查 stderr？

记录日期：2026-09-15。

**题目：** 在 `tests/test_learning_cli.py` 的成功场景中，为什么除了验证 stdout 中的 JSON 是空列表，还要验证 `result.stderr == ""`？

**学习者回答：** “因为虽然正确返回了结果，但我们仍然要检查期间有没有别的运行上的错误。”

**判断：** 理解方向正确。本题概念通过；本次回答没有提供 `2 passed` 等运行摘要，不能据此验收实际捕获测试。

**标准解析：** 仅验证 stdout 可以解析为预期 JSON，不会检查另一条输出通道。程序可能在 stdout 输出正确的空列表，同时在 stderr 写入意外的错误说明或警告。本项目这个成功场景约定 stderr 为空，因此要单独断言它，与退出码、stdout 一起验证完整对外行为。
...
- 这三个断言各检查一个方面，不能相互替代。
- stderr 为空不证明程序没有任何缺陷；静默的逻辑错误仍需其他测试发现。
- stderr 非空也不是所有命令都执行失败：一些 CLI 会在成功时把日志、进度或警告写入 stderr。本题的空通道要求来自本项目该场景的约定，不是所有程序的普遍规则。

**后续确认：** 学习者随后明确确认 `python -m pytest tests/test_learning_cli.py -q` 符合 `2 passed`，见 1D-Q7。保留本题最初仅回答概念、运行反馈稍后补齐的事实。


### 1D-Q7：CLI 自动捕获测试是否通过？

记录日期：2026-09-15。

**确认问题：** 执行 `python -m pytest tests/test_learning_cli.py -q` 后，是否显示 `2 passed`？

**学习者反馈：** “对的，结果跟你预测的一样。”

**判断：** 明确确认该命令结果符合 `2 passed`，本次 CLI 捕获实操通过；依据学习者反馈记录，不声称独立读取了其本机文件或完整日志。

**标准结果：**

| 用例 | 退出码断言 | stdout 断言 | stderr 断言 |
|---|---|---|---|
| k=3，无词表命中 | 0 | 可解析为 JSON 空列表 | 空 |
| k=-2，非法参数 | 2 | 空 | 包含正整数错误说明 |

两个参数化用例通过，pytest 摘要为 `2 passed`。这只表示当前文件中的两个场景通过，不是整个仓库的测试总数，也不是所有命令路径都已覆盖。

**复习要点：** 对整个 CLI 的测试不仅检查函数返回值，还执行真实子进程，检查退出码、输出通道和 JSON 格式，能发现直接调用函数的单元测试不一定覆盖的接口问题。
...
**结论：基础目标通过，进入同一阶段内的 1E。阶段 1 仍未整体验收。**

验收依据：

- 已运行最小 pytest 用例，观察大小写回归的通过→失败→恢复通过，并解释它发现的具体错误。
- 已理解 `pytest.raises` 的预期异常检查，运行非法 k 参数化测试，并正确预测混入合法值的后果。
- 已纠正类型注解与 argparse 实际转换的混淆，能区分整数转换与正整数范围校验。
- 已理解正常空结果与命令错误不能混为一谈，并确认真实 CLI 两个场景的退出码、stdout 和 stderr 自动断言通过。
- 已按给定步骤使用现有环境与开发依赖运行项目和测试；未要求从零独立实现。

范围说明：根据概念回答、纠正与学员运行反馈验收，不将导师自身检查替代学员掌握。本次基础验收不是全仓库回归测试，也不宣称所有操作系统、非空命中序列化、缺失文件、其他参数类型等路径均已逐项验证；这些现有或可扩展用例保留作复习与回归覆盖。下一课开始检索质量评测，不能因测试通过就宣称检索效果达标。

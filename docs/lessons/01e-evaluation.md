...
- **学习进度：** 进行中；单题 Recall@k 题、MRR 不应删去未召回可回答题的判断已答对；导师已补充统计范围的准确解释。无答案题空返回率与单指标局限题已纠正并通过；八题基线运行与 q06 定位已获正确反馈，导师补充了漏检根因与指标含义的区别；反例实操与阶段反思尚未验收。
- **前置理解：** 1D：CLI 与测试可运行
...
以上为导师在隔离副本上复核的教学预期，学习者尚未反馈本轮实操。题集已改变，不能将前后分数差直接称为同一测试集上的算法退步；本次是扩展评测覆盖，而不是修改检索算法。

完成后反馈：新增题的返回文档 ID 与分数、三个总体指标，以及“为何原八题空返回率 1 仍不足以说明能处理所有无答案问题”的解释。不贴整份报告也可以。阶段 1 尚未整体验收，暂不进入阶段 2。
...
**记录范围：** 基线结果依据学员的摘要反馈，导师未读取其完整终端日志；q06 根因是导师补充讲解。尚未收到九题反例副本的运行结果，不据此宣布 1E 或阶段 1 已通过。

...
- **学习进度：** 基础目标已完成。指标问答、纠错、八题基线与九题反例均已收到反馈，阶段 1 基础验收通过；[验收记录](../reviews/phase-01-evidence-baseline.md)。阶段 2 尚待学习者确认。
- **前置理解：** 1D：CLI 与测试可运行
...
学习者已反馈两组指标符合预期，并定位 q06；原答与补充根因分析见 1E-Q4。运行结果基于学习者反馈记录，不声称导师已读取其本机完整 JSON。

## 加入“有关键词但无依据”的反例（已完成，保留步骤供复习）

原八题中两道无答案题不含固定领域词，空返回率 1 不足以证明系统能处理更难的无答案问题。现在只扩展评测副本，不修改原题集、手册、词表或检索实现。

1. 新建目录 `artifacts/lesson-1e`。把 `data/questions.jsonl` **复制**为 `artifacts/lesson-1e/questions-with-counterexample.jsonl`，保留原来的八行。
2. 在副本末尾另起一行，粘贴下面完整的 JSON 对象。q09 只添加一次；JSONL 每行一个对象，不给整份文件加外层数组或行间逗号。

```jsonl
{"id":"q09","question":"Webhook 重试收费多少？","answerable":false,"relevant_document_ids":[],"reference_facts":[],"category":"unanswerable_with_terms"}
```

这里的 `false` 是 JSON 的布尔写法。该问题含领域词，但当前手册没有重试收费依据，因此按本题集约定标注为无答案；不能把“没有写收费”改成“免费”。
...
`--questions` 指定评测副本路径。输出应含九道题；`recall`、`mrr` 仍约为 0.833333，无答案题空返回率预期变为 2/3，约 0.666667。查看 q09 的 `retrieved`、`scores`、`recall`，注意单题词覆盖分数不等于回答依据充分。

以上教学预期已由导师在隔离副本上复核，学习者随后也反馈指标及 q09 文档与分数符合预期，见 1E-Q5。题集已改变，不能将前后分数差直接称为同一测试集上的算法退步；本次是扩展评测覆盖，而不是修改检索算法。

学习者已完成上述运行反馈，并指出 q09 无答案却仍返回候选，见 1E-Q5。阶段 1 基础验收已完成，下一阶段仍需学习者确认，不能因教材已存在就自动进入。
...
### 1E-Q4：八题基线与 q06 漏检定位

记录日期：2026-09-17。

**题目与实操：** 分别运行 `python -m evidencedesk.evaluate --k 1` 与 `--k 3`，核对总体指标，在 `details` 中找到可回答却 Recall=0 的题，并解释增大 k 为何没有解决它。

**学习者反馈：**

1. “符合预期。”
2. “应该是q06漏检”，并给出问题原文“消息没送到还会再发吗？”。
3. “应该是计算recall分子为0，意味着没有任何匹配。”

**判断：** 运行结果与题目定位正确。第三点描述了结果，但尚未完整解释根因；导师补充区分“没有找回相关文档”与“没有返回任何候选”，不把补充分析冒充学习者已独立给出的解释。

**按学习者反馈记录的基线：**

| k | Recall 宏平均（6 道可回答题） | MRR（6 道可回答题） | 无答案题空返回率（2 道题） |
|---|---|---|---|
| 1 | 5/6，约 0.833333 | 5/6，约 0.833333 | 1 |
| 3 | 5/6，约 0.833333 | 5/6，约 0.833333 | 1 |

两次 q06 的 `retrieved` 均为空列表，`scores` 为空列表，`recall` 为 0。其可回答标注假设上下文已明确讨论 Webhook，独立句子中的“消息”仍有歧义。

**导师补充：指标现象不等于根因。**

- Recall 分子为 0，仅说明返回结果与相关文档集合的交集为空。例如相关 ID 是 `{"doc-a"}`，返回 `["doc-b"]`，分子也为 0，但并不是没有候选。
- q06 的实际返回确实为空，根因是固定词表没有识别“消息”“送到”“再发”这些表达。`tokens("消息没送到还会再发吗？")` 得到空集合。
- `search()` 在查询词集合为空时提前返回 `[]`，尚未进入评分、排序与 top-k 截取。增大 k 不会增加领域词，也不会产生语义匹配。
- 因此本例可连成因果链：词表未覆盖表达 → 查询词为空 → 提前返回空候选 → 没有相关文档被找回 → 单题 Recall/RR 为 0。不能反过来把分子为 0 本身当成检索漏掉的原因。

**复习要点：** 先用指标定位问题，再看逐题输出和实现找原因。不能为了消除失败而删掉 q06、改金标准或把参考答案传给检索器。这八题是公开开发样例，分数不是对未见问题的泛化保证，也不是生成答案正确率。

**记录范围：** 基线结果依据学员的摘要反馈，导师未读取其完整终端日志；q06 根因是导师补充讲解。在这道题作答时，九题反例尚未反馈；后续反馈与验收结论见 1E-Q5 和阶段验收记录，不将后续完成情况倒写为本题当时已完成。


### 1E-Q5：加入带关键词的无答案题后，指标怎样变化？

记录日期：2026-09-17。

**题目与实操：** 在原八题的副本中添加 q09“Webhook 重试收费多少？”，标注无答案，以 k=3 评测。反馈三个总体指标、q09 返回的文档与分数，并解释为何原八题空返回率 1 不能证明能处理所有无答案问题。

**学习者反馈：**

> 1. 符合预期，q09返回了"webhook-delivery"，分数为1
> 2. 因为q09是不可回答的，然而结果却正常返回了

**判断：** 正确。已确认运行结果符合预期，并识别新增无答案题仍返回候选这一现象。导师进一步澄清“正常返回”的含义：CLI 正常完成不等于检索质量合格，返回候选也不等于已生成答案。

**标准结果：**

| 项目 | 原八题，k=3 | 加入 q09 后九题，k=3 |
|---|---|---|
| 可回答题数 | 6 | 6 |
| 无答案题数 | 2 | 3 |
| Recall 宏平均 | 5/6 | 5/6 |
| MRR | 5/6 | 5/6 |
| 无答案题空返回率 | 2/2 = 1 | 2/3，约 0.666667 |

新增 q09 的 `retrieved` 为 `["webhook-delivery"]`，`scores` 为 `[1.0]`，`recall` 为 `null`。`null` 来自无答案分组，不表示“满分”或“零分”。

**标准解析：** 原两道无答案题都没有命中固定词表，因此容易返回空。q09 含有 `webhook`、`重试`，能以词覆盖率 1 命中文档，但手册没有重试收费依据；所以它未达到本评测对无答案题的空返回目标。三个无答案题中仍只有两个返回空，得到 2/3。算法未变、评测题集扩大，不能将这次差异称为同一测试集上的算法退步。

**复习要点：**

- 区分软件执行成功、检索结果满足评测目标、最终答案有证据支持这三件事。
- 本阶段只返回候选文档，没有生成答案，不能仅据非空返回就宣称模型已经编造答案。
- 词覆盖率高不保证文档含有所需事实；单个数据集的高指标也不能保证所有新问题表现良好。
- 评测需要覆盖有关键词但缺事实的无答案题，同时保留可回答题；不能只挑让系统容易成功的样例。

**记录范围：** 学习者已反馈摘要和解释，未提供完整本机日志或副本文件；导师的隔离副本复核是独立参考检查，不替代学习者记录。原始八题和生产检索代码未为这次课程更新而修改。

## 1E 与阶段 1 验收小结

本课基础目标通过。结合 1A–1D 的概念回答、纠错和关键实操反馈，阶段 1 的学习基础目标已验收；完整依据、复现命令和限制见[阶段 1 验收记录](../reviews/phase-01-evidence-baseline.md)。

这不表示检索器已无缺陷、课程后续阶段已通过或生产系统已完成。q06 的词表漏检、q09 的无依据候选保留为已知限制；部分根因分析由导师补充，不宣称学习者已独立完成所有分析。

**在这里等待学习者确认。** 下一阶段准备处理长文分块、来源和更新；分块本身不保证解决同义表达与证据充分性问题，语义检索和生成检查会在后续环节继续展开。

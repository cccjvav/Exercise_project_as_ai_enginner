...
> **2026-09-23 决定（学习者选择）：真实模型走免费 API 路线。** 生成/裁判首选 Agnes `agnes-2.5-flash`，备选 Gemini Flash-Lite 免费层与 Groq 免费开发者层（仅聊天）；embedding 首选 Gemini 免费层 embedding 模型，不可用时核实 Agnes 是否提供，再不行退回本机 `sentence-transformers` 小模型。已知约束：Google 已不再公布免费层固定限额，配额以 AI Studio 显示为准且不保证；免费层数据可能用于训练，因此只发送虚构语料；Groq 无 embedding。进入 3A 前的准备清单见[修订记录 §2.3](course-revision-2026-09-23.md)。以下正文为 2026-09-13 的核实记录，保留不改。
